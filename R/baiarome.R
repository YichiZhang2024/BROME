...
#'
#' \code{baiarome} implements the Bayesian ROME framework with alignment (ABROME)
#' with Bayesian Alignment and Approximate Invariance
#'
...
baiarome <- function(pninv, n_items, n_ninv, data, ROME_lim, eta_lim, eta_num){
  ...
  rome <- ROME_lim * pvar
  ## step 2 fit bayesian alignment with small variance priors
  ba_res <- run_ba(data = data, svp = TRUE, n_items = n_items, n_ninv = n_ninv)
  ## step 3 compute expected total scale scores
